...
func IsActiveOrInactive(value bool) fiber.Handler {
	return func(ctx *fiber.Ctx) error {
		...
		if userDto.IsActive == value {
			...
			return  ctx.Status(401).JSON(fiber.Map{"error": fmt.Sprintf("it was supposed your user to be is_active: %t", value)})
		}
	}
}